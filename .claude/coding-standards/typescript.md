...
async function processOrder(orderId: string): Promise<void> {
  try {
    ...
    console.error(error); // Logged but caller has no idea!
  }
}
...
async function processOrder(orderId: string): Promise<void> {
  try {
    ...
    throw new OrderProcessingError(`Failed to process order ${orderId}`, { cause: error });
  }
}

// ✅ GOOD - API endpoint returns error to client
app.post("/orders/:id/process", async (req, res) => {
  try {
    await orderService.process(req.params.id);
...
Frontend errors MUST be logged to console AND shown as toast:
```typescript
// ❌ BAD - Silent catch
...
// ❌ BAD - Console only (user can't see console)
async function loadUserData(): Promise<void> {
  try {
    await fetchUser();
  } catch (error) {
