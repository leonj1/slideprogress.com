...
### Backend Error Handling (NEVER Swallow Errors)
Backend errors MUST be propagated to the caller/client:
```go
// ❌ BAD - Swallowing error (caller never knows!)
func ProcessOrder(ctx context.Context, orderID string) {
    err := orderService.Process(ctx, orderID)
    if err != nil {
        log.Printf("Error: %v", err) // Logged but caller has no idea!
    }
}
...
// ✅ GOOD - HTTP handler returns error to client
func ProcessOrderHandler(w http.ResponseWriter, r *http.Request) {
    orderID := chi.URLParam(r, "orderID")
    err := orderService.Process(r.Context(), orderID)
    if err != nil {
...
### Frontend Error Handling (Log + Display)
For Go frontend frameworks (Wails, Fyne, etc.), errors MUST be logged AND shown to user:
```go
// ❌ BAD - Silent error handling
func (a *App) LoadData() {
    _, err := a.service.FetchData()
    if err != nil {
...
// ❌ BAD - Logging only
func (a *App) LoadData() {
    data, err := a.service.FetchData()
    if err != nil {
        log.Printf("Error: %v", err) // User can't see logs!
    }
}
