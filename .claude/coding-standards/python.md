...
def process_order(order_id):
    try:
        ...
        logger.error(f"Error: {e}")  # Logged but caller has no idea!
...
# ✅ GOOD - FastAPI endpoint returns error to client
@app.post("/orders/{order_id}/process")
async def process_order_endpoint(order_id: str):
    # Let exceptions propagate - global handler converts to HTTP response
    return order_service.process(order_id)
```

### Frontend Error Handling (Console + Toast)
For Python frontend frameworks (Streamlit, Dash, etc.), errors MUST be logged AND shown to user:
```python
# ❌ BAD - Silent catch
def load_data():
    try:
        ...
        pass  # User has no idea something failed!
...
def load_data():
    try:
        ...
        logger.error(e)  # User can't see logs!
        return None

# ✅ GOOD - Log + Show user notification
def load_data():
    try:
        return fetch_data()
    except Exception as e:
        logger.error(f"Failed to load data: {e}")
        st.error("Failed to load data. Please try again.")  # Streamlit toast
        return None
```
