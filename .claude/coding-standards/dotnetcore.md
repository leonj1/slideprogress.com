...
### Backend Error Handling (NEVER Swallow Exceptions)
Backend exceptions MUST be propagated to the caller/client:
```csharp
// ❌ BAD - Swallowing exception (caller never knows!)
public async Task ProcessOrderAsync(string orderId)
{
    ...
    {
        _logger.LogError(ex, "Error processing order"); // Logged but caller has no idea!
    }
}
...
// ✅ GOOD - Controller lets exception propagate to middleware
[HttpPost("{orderId}/process")]
public async Task<IActionResult> ProcessOrder(string orderId)
{
    // Let exceptions propagate - global middleware converts to HTTP response
    await _orderService.ProcessAsync(orderId);
    return Ok(new { success = true });
}
```

### Frontend Error Handling (Console + Toast)
For .NET frontend frameworks (Blazor, MAUI, WPF), errors MUST be logged AND shown to user:
```csharp
// ❌ BAD - Silent catch
...
private async Task LoadDataAsync()
{
    ...
    {
        _logger.LogError(ex, "Failed to load data"); // User can't see logs!
    }
}

// ✅ GOOD - Log + Show user notification (Blazor example)
private async Task LoadDataAsync()
{
    try
    {
    ...
    catch (Exception ex)
    {
        ...
        await _toastService.ShowErrorAsync("Failed to load data. Please try again.");
    }
}

// ✅ GOOD - Log + Show user notification (MAUI example)
private async Task LoadDataAsync()
{
    try
    {
